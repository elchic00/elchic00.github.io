...
export const AIChatAssistant = () => {
  ...
  // Track session start time when chat opens
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === "d") {
        ...
        setIsOpen((prev) => {
          const newState = !prev;
          if (newState) {
            // Track when opened via keyboard
            const userMessages = messages.filter((m) => m.role === "user").length;
            trackChatOpened(userMessages === 0, userMessages);
          }
          return newState;
        });
      }
  ...
  }, [isOpen, input, messages]);

  useEffect(() => {
    const handleOpenChat = () => {
      setIsOpen(true);
      // Track when opened via custom event
      const userMessages = messages.filter((m) => m.role === "user").length;
      trackChatOpened(userMessages === 0, userMessages);
    };
  ...
  const handleClose = useCallback(() => {
    ...
    if (sessionStartTimeRef.current) {
      ...
      const userMessages = messages.filter((m) => m.role === "user").length;
      trackChatSessionEnd(messages.length, sessionDuration, userMessages);
  ...
  const sendMessage = async (messageText: string) => {
    ...
    // Track message sent
    trackMessageSent(userMessage.length, userMessageCount);
  ...
  const handleSuggestedQuestion = useCallback(
    (question: string, index: number, total: number) => {
      trackSuggestionClicked(index, total);
  ...
  const handleClearChat = useCallback(() => {
    trackChatCleared(messages.length);
  ...
  const handleQuickAction = useCallback(
    (action: string) => {
      trackQuickActionClicked(action);
      handleAction(action, handleClose);
    },
    [handleClose]
  );
